...
public class UserServiceImpl implements UserService {
	...
	public void registerUser(SignupDto signupDto) {
		...
		try {
			...
			Users users = Users.builder().firstName(signupDto.getFirstName()).lastName(signupDto.getLastName())
					.iamId(iamId).birthDate(signupDto.getBirthDate()).createdOn(Instant.now())
					.email(signupDto.getEmail()).roleId(1).subscriptionId(1L).emailVerified(false)
					.isMessenger(signupDto.getIsMessenger()).deceased(false).isFirstLetterCreated(false)
					.isFirstVideoCreated(false).isFirstAudioCreated(false).hasWritten(false).build();
			...
			if (signupDto.getIsMessenger()) {
				...
				Optional<Messenger> messenger = messengerRepository
						.findByInvitationToken(signupDto.getInvitationToken());
			...
			EmailMessage emailMsg = getEmailMessagePojo(fromAddress, users.getEmail(),
					LFareWellConstants.SIGN_UP_SUBJECT, LFareWellConstants.SIGN_UP_TEMPLATE, users.getFirstName()
							+ (StringUtils.isNotEmpty(users.getLastName()) ? " " + users.getLastName() : ""),
					token);
	...
	private EmailMessage getEmailMessagePojo(String from, String to, String subject, String templateName,
	...
	public void forgotPassword(String email) {
		...
		EmailMessage emailMsg = getEmailMessagePojo(fromAddress, user.getEmail(),
				...
				user.getFirstName() + (StringUtils.isNotEmpty(user.getLastName()) ? " " + user.getLastName() : ""),
				token);
	...
	public UserDetailsDto getUser() {
		...
		Users user = usersRepository.findByIamId(iamId).orElseThrow(() -> new UserException("User Not Found"));
		...
		if (user.getSubscriptionId() == null) {
			userDetailsDto.setSubscription(SubscriptionDto.builder().subscriptionId(1L).name("Freemium")
					.features(List.of(FeatureDto.builder().id(1L).name("last_letters").build(),
							FeatureDto.builder().id(2L).name("last_videos").build(),
							FeatureDto.builder().id(3L).name("last_audios").build(),
							FeatureDto.builder().id(4L).name("messengers").build(),
							FeatureDto.builder().id(5L).name("memorial_page").build(),
							FeatureDto.builder().id(6L).name("photos").build()))
					.plan(PlanDto.builder().id(1L)
							.photos(MediaData.builder().dataCountLimit(-1).uploadSizeLimit(5).build())
							.lastAudios(AudioSettings.builder().lengthLimit(3).dataCountLimit(3).uploadSizeLimit(25)
									.build())
							.lastVideos(VideoSettings.builder().lengthLimit(2).dataCountLimit(3).uploadSizeLimit(50)
									.build())
		...
		userDetailsDto
				.setMessagesCount(MessageCountDto.builder().letters(Long.valueOf(lastMessageCount.getLetterCount()))
						.videos(Long.valueOf(lastMessageCount.getVideoCount()))
						.audios(Long.valueOf(lastMessageCount.getAudioCount())).build());
		return userDetailsDto;
	}

	@Override
	public Users updateUser(Long id, UpdateUserDto updateUserDto) {
		Users user = usersRepository.findById(id).orElseThrow(() -> new UserException("User not found"));
	...
	@Override
	public void updateUserPassword(@Valid UpdatePasswordDto updatePasswordDto) {
		try {
			...
			loginDto.setPassword(updatePasswordDto.getCurrentPassword());
			keycloakService.login(loginDto);
			Users user = usersRepository.findByEmail(userEmail).get();
			if (user != null) {
				keycloakService.updatePassword(user.getIamId(), updatePasswordDto.getNewPassword());
				EmailMessage emailMsg = getEmailMessagePojo(fromAddress, user.getEmail(),
						LFareWellConstants.UPDATE_PASSWORD_SUBJECT, LFareWellConstants.UPDATE_PASSWORD_TEMPLATE,
						user.getFirstName()
			...
			log.error("Incorrect current password.", ex);
			throw new IAMException("Incorrect current password.");
		} catch (Exception e) {
	...
	@Override
	public Users updateSubscription(Long id) {
		Optional<Users> user = usersRepository.findById(id);
		if (user.isPresent() && user.get().getSubscriptionId() == 1L) {
			// TODO This logic will change in future when multiple subscriptions and plans
			// are added
			log.info("Updating user {} subscription from Freemium to Legacy Edition", id);
			user.get().setSubscriptionId(2L);
			user.get().setUpdatedOn(Instant.now());
	...
	@Override
	public Integer walkthroughCountIncrement(Long id) {
		Users user = usersRepository.findById(id).orElseThrow(() -> new UserException("User not found"));
		int count = user.getWalkthroughCount() + 1;
		user.setWalkthroughCount(count);
		user.setUpdatedOn(Instant.now());
		usersRepository.save(user);
		return count;
	}
