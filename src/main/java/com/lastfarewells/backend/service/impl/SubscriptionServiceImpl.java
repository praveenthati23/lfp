...
public class SubscriptionServiceImpl implements SubscriptionService {
    ...
    public SubscriptionDto fetchSubscriptionDetails(Long subscriptionId) {
        ...
        Map<String, Object> plaMap = planMapper(subscription.getPlan());

        PlanDto planDto = PlanDto.builder().id(subscription.getPlan().getId()).build();
        planDto.setPhotos(plaMap.get("photos"));
        planDto.setLastVideos(plaMap.get("lastVideos"));
        planDto.setLastAudios(plaMap.get("lastAudios"));
        planDto.setMemorialPage(plaMap.get("memorialPage"));
        planDto.setLastLetters(plaMap.get("lastLetters"));
    ...
    private Map<String, Object> planMapper(Plan plan) {
        try {
            return mapper.readValue(plan.getExtras(), Map.class);
